...
export class PetraAccount extends WebAccount {
    ...
    async walletSignTxnImpl(txn) {
        ...
        try {
            const signedPayload = await this.wallet.signTransaction(txn.payload, signingOption);
            const deserializer = new BCS.Deserializer(Uint8Array.from(Object.values(signedPayload)));
            const signedTxn = TxnBuilderTypes.SignedTransaction.deserialize(deserializer);
            return signedTxn;
        }
        catch (e) {
            console.error(e);
...
//# sourceMappingURL=PetraAccount.js.map